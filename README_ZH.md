...
独立的项目设计文档，适用对象： **Sid Meier's Civilization VI**。
...
- **回合经济工作表** (Turn economy worksheets) — 规划模块，尚未实现。
- **科技路线比较** (Technology route comparisons) — 规划模块，尚未实现。
- **规则集兼容性说明** (Ruleset compatibility notes) — 规划模块，尚未实现。
...
| 字段 | 内容 |
|---|---|
| 适用对象 | Sid Meier's Civilization VI |
| 类别 | 本地游戏场景规格 |
| 输入 | 手工记录或用户授权的本地导出 |
| 计划输出 | 回合经济工作表 |
| 兼容性 | 未验证；未声明支持任何具体版本 |
